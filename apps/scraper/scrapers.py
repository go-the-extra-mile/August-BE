...
class UMDScraper:
    ...
    def get_department_open_sections(self, sem: int, dep: str):
        ...
        for course_div in course_divs:
            ...
            code = course_div.select_one(
                "div.course-id-container.one.columns div.course-id"
            )
            if code is None:
                continue
            code = code.string
            name = course_div.select_one(
                "div.course-info-container.eleven.columns div.course-basic-info-container.sixteen.colgrid span.course-title"
            )
            if name is None:
                continue
            name = name.string
            credits = course_div.select_one(
                "div.course-info-container.eleven.columns div.course-basic-info-container.sixteen.colgrid span.course-min-credits"
            )
            if credits is None:
                continue
            ...
            notes = str()
            course_detail = course_div.select(
                "div.course-info-container.eleven.columns > div.approved-course-texts-container > div:nth-child(1) > div > div > div > div"
            )
            ...
            # Gen Ed
            gen_ed = course_div.select(
                "div.course-info-container.eleven.columns > div.course-basic-info-container.sixteen.colgrid > div.course-stats-container.row.clearfix > div.gen-ed-codes-group.six.columns > div"
            )
            if len(gen_ed) > 0:
            ...
            # Core
            core = course_div.select(
                "div.course-info-container.eleven.columns > div.course-basic-info-container.sixteen.colgrid > div.course-stats-container.row.clearfix > div.core-codes-group.two.columns > div"
            )
            if len(core) > 0:
            ...
            course_text = course_div.select(
                "div.course-info-container.eleven.columns > div.course-texts-container > div > div > div"
            )
            ...
            sections = []
            sections_container_div = course_div.select_one(
                "div.course-info-container.eleven.columns > div.toggle-sections-link-container > div > div > fieldset > div > div.sections"
            )
            ...
            for section_div in sections_div:
                ...
                section_info_div = section_div.select_one(
                    "div.section-info-container > div"
                )
                section["code"] = str(
                    section_info_div.find("span", class_="section-id").string
                ).strip()
                section["code"] = course["code"] + "-" + section["code"]
                instructors_span = section_info_div.find(
                    "span", class_="section-instructors"
                ).find_all("span", class_="section-instructor")
                section["instructors"] = list()
                for inst in instructors_span:
                    section["instructors"].append(str(inst.string).strip())
                seats_info_span = section_info_div.select_one(
                    "div.seats-info-group.six.columns > div > span.seats-info"
                )
                seats = seats_info_span.select_one(
                    "span.seats-info span.total-seats-count"
                )
                if seats is not None:
                    section["seats"] = int(seats.string)
                open_seats = seats_info_span.select_one(
                    "span.open-seats span.open-seats-count"
                )
                if open_seats is not None:
                    section["open_seats"] = int(open_seats.string)
                waitlist_holdfile = seats_info_span.select(
                    "span.waitlist span.waitlist-count"
                )
                ...
                section_meetings_div = section_div.select_one(
                    "div.class-days-container"
                )
                if section_meetings_div is None:
                    continue
                section_meetings_div = section_meetings_div.find_all(
                    "div", class_="row"
                )
    ...
    def regularize_meetings(self, meetings):
        ...
        for m in meetings:
            ...
            start_time = (
                ...
                else self.to_time(m.get("start_time"))
            )
            end_time = (
                None if m.get("start_time") is None else self.to_time(m.get("end_time"))
            )
            bldg = "ONLINE" if m.get("room") == "ONLINE" else m.get("bldg")
            room = (
                ""
                if (m.get("room") == "ONLINE" or not m.get("room"))
                else m.get("room")
            )
            for d in days_in_m:
                reg_meeting = {
    ...
    def save(self, semester: int, open_sections_data: list[dict]):
        ...
        institution, _ = Institution.objects.get_or_create(
            full_name=self.INSTITUTION_FULL_NAME, nickname=self.INSTITUTION_NICKNAME
        )
            ...
            for section in crs.get("sections"):
                ...
                for m in meetings:
                    ...
                    loc, _ = Location.objects.get_or_create(
                        building=bldg, room=m.get("room")
                    )
                    dur, _ = Duration.objects.get_or_create(
                        start_time=m.get("start_time"), end_time=m.get("end_time")
                    )
                    day, _ = Day.objects.get_or_create(day=m.get("day"))
                    latest_m, _ = Meeting.objects.get_or_create(
                        duration=dur,
                        day=day,
                        location=loc,
                ...
                old_meetings = set(
                    Meeting.objects.filter(opened_section=opened_section)
                )
...
scrapers = {"University of Maryland": UMDScraper()}
