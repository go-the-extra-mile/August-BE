...
class OpenedSectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["section_code", "meetings", "instructors", "credits",]


class MergedMeetingsOpenedSectionSerializer(OpenedSectionSerializer):
    meetings = serializers.SerializerMethodField(
        method_name="get_meetings_with_merged_days"
    )
...
class SimpleOpenedSectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OpenedSection
        fields = ("id", "section_code", "seats", "open_seats", "waitlist", "holdfile", "meetings_exist")
        read_only_fields = ["section_code", "meetings_exist"]
...
class InstructorSectionSerializer(BaseInstructorSerializer):
    ...
    class Meta(BaseInstructorSerializer.Meta):
        fields = BaseInstructorSerializer.Meta.fields + ("sections",)
...
class CourseSectionByInstructorSerializer(BaseCourseSerializer):
    ...
    def get_sections_by_instructor(self, course):
        ...
        if course_opened_sections is None:
            return None
        ...
        for instructor in instructors:
            instructor_sections = course_opened_sections.filter(
                teach__instructor=instructor
            )

            res.append(
                InstructorSectionSerializer(
                    instructor, context={"instructor_sections": instructor_sections}
                ).data
            )
...
class CourseSectionSerializer(BaseCourseSerializer):
    ...
    def get_sections(self, course):
        ...
        if course_opened_sections is None:
            return None
        return MergedMeetingsOpenedSectionSerializer(
            course_opened_sections, many=True
        ).data
